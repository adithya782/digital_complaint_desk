...
window.onload = function () {
  ...
  google.accounts.id.initialize({
    client_id:
      "367509399123-519vrqo9a4813imrbfu8npacn7qordjs.apps.googleusercontent.com", // ⚠️ Paste your real Client ID string here!
    callback: handleGoogleCredentialResponse,
  });

  // 2. Render the official button layout with custom branding text options
  google.accounts.id.renderButton(document.getElementById("googleButtonDiv"), {
    theme: "outline",
    size: "large",
    ...
    .then((data) => {
      if (data.access_token) {
        ...
        if (data.registration_incomplete) {
          alert(
            "Google Sign-In successful! Please complete your profile next.",
          );
          ocalStorage.setItem("temp_oauth_email", data.email);
